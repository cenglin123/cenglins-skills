...
> ⚠️ **改源代码后必须同步副本**。本仓库是唯一事实源；各框架 skill 目录（`~/.agents/skills/`、`~/.claude/skills/`）若是**拷贝**安装的，不会随源仓库自动更新——`git pull` 或直接编辑源 `SKILL.md` 后，必须重新拷贝，否则 agent 加载的仍是旧副本。
>
> **优先用符号链接**（自动同步，`git pull` 后即生效）。只能拷贝时，每次改源后重新拷贝并核对。真实事故：crlf-guard 源仓库已修复 bug 命令，但 `~/.agents/skills/crlf-guard` 拷贝副本未同步，agent 继续加载旧版本，把一个项目的 CHANGELOG.md 清空成 0 字节。

更新后，凡是以**拷贝**方式安装的 skill 需重新拷贝一次；符号链接方式则自动生效。

**核对同步状态**（对比源与各部署副本的 hash，输出 DRIFT 项）：

```powershell
$src = "$env:USERPROFILE\.agents\cenglins-skills"
foreach ($d in @("$env:USERPROFILE\.agents\skills","$env:USERPROFILE\.claude\skills")) {
  Get-ChildItem $src -Recurse -Filter SKILL.md | ForEach-Object {
    $name = ((Get-Content $_.FullName -TotalCount 3 | Select-String '^name:') -replace '^name:\s*','').Trim()
    $dst = Join-Path $d "$name\SKILL.md"
    if ((Test-Path $dst) -and ((Get-FileHash $_.FullName).Hash -ne (Get-FileHash $dst).Hash)) { Write-Output "DRIFT: $name @ $d" }
  }
}
```
...
New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.claude\skills\crlf-guard" -Target "$env:USERPROFILE\.agents\cenglins-skills\01-guards\crlf-guard"
```
...
`~/.agents/skills/` 由用户手动维护：从仓库把叶子目录链接或拷贝过去。**推荐符号链接**（源仓库更新后自动同步，避免拷贝漂移）：

```powershell
# 推荐：符号链接（git pull 后自动同步）
New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.agents\skills\utf8-guard" -Target "$env:USERPROFILE\.agents\cenglins-skills\01-guards\utf8-guard"
New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.agents\skills\powershell-guard" -Target "$env:USERPROFILE\.agents\cenglins-skills\01-guards\powershell-guard"
New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.agents\skills\crlf-guard" -Target "$env:USERPROFILE\.agents\cenglins-skills\01-guards\crlf-guard"
```

无法创建符号链接时改用拷贝（**注意：源仓库每次改动后必须重新拷贝**，见上节"更新"）：
